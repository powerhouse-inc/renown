...
/** Shared "Sign in with Renown" card shell used by every non-`invalid` view. */
function OidcLoginCard({ children }: { children: React.ReactNode }) {
    return (
...
const OidcLoginFlow: React.FC<OidcLoginFlowProps> = ({ requestId, issuer }) => {
    ...
    if (view.kind === "loading") {
        return (
            <OidcLoginCard>
                <p className="text-center text-lg leading-6 text-muted-foreground-light">Loading…</p>
            </OidcLoginCard>
        );
    }
    ...
    return (
        <OidcLoginCard>
            <p className="mb-6 text-center text-lg leading-6 text-muted-foreground-light">
                <strong>{client.name}</strong> at <strong>{client.redirectHost}</strong> wants to
                confirm your Renown identity.
            </p>
